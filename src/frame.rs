...
impl<'a> Frame<'a> {
    ...
    pub fn buffer_crop(
        ...
        log::info!(
            "wc: обрезка {start_width},{start_height} .. {end_width},{end_height}              при кадре {}x{}",
            self.width,
            self.height
        );
        ...
        // Проверка, которой в апстриме нет: область обрезки не должна вылезать
        // за исходную текстуру. Иначе копирование области уходит за её пределы
        // и рушит кучу (STATUS_HEAP_CORRUPTION), причём без единой строчки в
        // журнале — процесс умирает раньше, чем что-то успевает записаться.
        if end_width > self.width || end_height > self.height {
            log::error!(
                "wc: ОБРЕЗКА ВЫЛЕЗАЕТ ЗА КАДР — запрошено до {end_width}x{end_height},                  а кадр {}x{}",
                self.width,
                self.height
            );
            return Err(Error::InvalidSize);
...
impl<'a> FrameBuffer<'a> {
    ...
    pub fn as_nopadding_buffer(&mut self) -> Result<&mut [u8], Error> {
        ...
        // Апстрим сверяется с capacity, а пишет по длине — и дальше режет срез
        // по frame_size. Растим по длине, иначе при ёмкости больше длины срез
        // в конце паникует, а запись сырым указателем идёт по неинициали-
        // зированной памяти.
        if self.buffer.len() < frame_size {
            self.buffer.resize(frame_size, 0);
        }
        log::info!(
            "wc: разбор кадра {}x{}, шаг строки {}, нужно {frame_size} байт, буфер {}",
            self.width,
            self.height,
            self.row_pitch,
            self.buffer.len()
