        ...
        <Self as GraphicsCaptureApiHandler>::Flags: Send,
    {
        ...
        let thread_handle =
            thread::spawn(move || -> Result<(), GraphicsCaptureApiError<Self::Error>> {
                // Initialize WinRT
                log::info!("wc: поток захвата начал работу");
                static INIT_MTA: OnceLock<()> = OnceLock::new();
                INIT_MTA.get_or_init(|| {
                    unsafe {
                        CoIncrementMTAUsage().expect("Failed to increment MTA usage");
                ...
                log::info!("wc: 1/6 MTA поднята, инициализируем WinRT");
                match unsafe { RoInitialize(RO_INIT_MULTITHREADED) } {
                    Ok(_) => (),
                    Err(e) => {
                        if e.code() == S_FALSE {
                ...
                log::info!("wc: 2/6 WinRT готова, создаём очередь диспетчера");
                // Create a dispatcher queue for the current thread
                let options = DispatcherQueueOptions {
                    dwSize: u32::try_from(mem::size_of::<DispatcherQueueOptions>()).unwrap(),
                    threadType: DQTYPE_THREAD_CURRENT,
                ...
                log::info!("wc: 3/6 очередь создана, создаём устройство D3D11");
                // Create direct3d device and context
                let (d3d_device, d3d_device_context) = create_d3d_device()?;
                log::info!("wc: 4/6 устройство D3D11 создано");
                ...
                log::info!("wc: 5/6 цель преобразована, строим GraphicsCaptureApi");
                let mut capture = GraphicsCaptureApi::new(
                    d3d_device,
                    d3d_device_context,
                    item,
                ...
                log::info!("wc: 6/6 GraphicsCaptureApi построен, запускаем сеанс");
                capture
                    .start_capture()
                    .map_err(GraphicsCaptureApiError::GraphicsCaptureApiError)?;
                log::info!("wc: сеанс запущен, входим в цикл сообщений");
