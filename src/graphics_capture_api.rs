...
impl GraphicsCaptureApi {
    ...
    >(
        ...
        log::info!(
            "wc: размер цели захвата {}x{}",
            item_size.Width,
            item_size.Height
        );
        let frame_pool =
            Direct3D11CaptureFramePool::Create(&direct3d_device, pixel_format, 1, item_size)?;
        log::info!("wc: кадровый пул создан");
        let frame_pool = Arc::new(frame_pool);

        // Create capture session
        let session = frame_pool.CreateCaptureSession(&item)?;
        log::info!("wc: сеанс захвата создан");
        ...
        >::new({
            ...
            move |frame, _| {
                ...
                log::info!("wc: кадр пришёл, текстура {texture_width}x{texture_height}");
